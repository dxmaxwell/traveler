  ...
  $('#modify').click(function (e) {
    ...
    if (roles.sort().join() === currentRoles.sort().join()) {
      ...
      var request = $.ajax({
        ...
        $('#message').append('<div class="alert alert-info"><button class="close" data-dismiss="alert">x</button>The save request failed. You might need to try again or contact the admin.</div>');
      }).always(function () {});
    }
    e.preventDefault();
  });
